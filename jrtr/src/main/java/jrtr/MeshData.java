...
public class MeshData {
	...
	public void loop() {
		// subdivision
		List<Edge> subdividedEdges = new ArrayList<Edge>();
		int[] indices = new int[4*3*faceTable.size()]; // 4 triangles per face, 3 indices per triangle
		int i = 0;
		int vertexTableSize = vertexTable.size();
		// subdivide each face
		for(Face f : faceTable){
			for(Edge e : findEdges(f)){
				if(!subdividedEdges.contains(e)){
					Vector3f position = new Vector3f();
					position = vertexTable.get(e.v1).position;
					position.add(vertexTable.get(e.v2).position);
					position.scale(3f);

					// search for vertex in right wing
					for(Edge e1 : findEdges(e.f1)){
						// search for new edge containing the vertex
						if(!e1.equals(e)){
							// is it the new vertex
							if(e1.v1 != e.v1 && e1.v1 != e.v2){
								position.add(vertexTable.get(e1.v1).position);
								break;
							} else {
								position.add(vertexTable.get(e1.v2).position);
								break;
							}
						}
					}
					// search for vertex in left wing
					for(Edge e2 : findEdges(e.f2)){
						// search for new edge containing the vertex
						if(!e2.equals(e)){
							// is it the new vertex
							if(e2.v1 != e.v1 && e2.v1 != e.v2){
								position.add(vertexTable.get(e2.v1).position);
								break;
							} else {
								position.add(vertexTable.get(e2.v2).position);
								break;
							}
						}
					}
					...
					subdividedEdges.add(e);
				}
			}
			List<Integer> verticesOfFace = findVertices(f);
			
			indices[12*i] = verticesOfFace.get(0);
			indices[12*i+1] = vertexTableSize;
			indices[12*i+2] = vertexTableSize+1;
			
			indices[12*i+3] = verticesOfFace.get(1);
			indices[12*i+4] = vertexTableSize+1;
			indices[12*i+5] = vertexTableSize+2;
			
			indices[12*i+6] = verticesOfFace.get(2);
			indices[12*i+7] = vertexTableSize+2;
			indices[12*i+8] = vertexTableSize;
			
			indices[12*i+9] = vertexTableSize;
			indices[12*i+10] = vertexTableSize+1;
			indices[12*i+11] = vertexTableSize+2;
			i++;
		}
		
		createMesh(vertexTable, indices);
